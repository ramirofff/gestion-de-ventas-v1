    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 409 && data.existingAccount) {
          // Cuenta ya existe - mostrar estado actual
          setAccountData({
            accountId: data.existingAccount.stripeAccountId,
            businessName: data.existingAccount.businessName,
            status: data.existingAccount.status,
            email: data.email,
            message: `✅ Ya tienes una cuenta Stripe Connect registrada: "${data.existingAccount.businessName}"`
          });
        ...
        {step === 'form' && (
          <div className="bg-white rounded-lg shadow-lg p-8">
            ...
            <form onSubmit={createConnectedAccount} className="space-y-6">
              ...
              {/* Información especial para Argentina */}
              {accountForm.country === 'AR' && (
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  <h3 className="text-blue-800 font-semibold mb-2">
                    🇦🇷 Información importante para Argentina
                  </h3>
                  <ul className="text-blue-700 text-sm space-y-1">
                    <li>• Los pagos se procesan en tu cuenta principal de USA</li>
                    <li>• Se calcula automáticamente tu comisión del 5%</li>
                    <li>• Los transfers a Argentina se hacen manualmente</li>
                    <li>• No requiere proceso de verificación con Stripe</li>
                  </ul>
                </div>
              )}
